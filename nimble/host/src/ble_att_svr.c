...
static void pending_att_prepare(uint16_t conn_handle,
                                ...
                                uint8_t offset, struct os_mbuf *om,
                              ...
                              uint16_t cid)
{
    ...
    rc = ble_gap_authorize_event(conn_handle, attr_handle, access_opcode,
                                 cid);
...
ble_att_svr_create_read_rsp(uint16_t conn_handle, uint16_t cid,
                            struct os_mbuf *om, int hs_status, uint8_t att_op,
                            uint16_t err_handle, uint16_t offset,
                            uint8_t *out_att_err)
{
    int rc = 0;
    ...
    rc = ble_att_svr_read_handle(conn_handle, err_handle, offset, om,
                                 out_att_err);

done:
    rc = ble_att_svr_tx_rsp(conn_handle, cid, rc, om, att_op,
                            *out_att_err, err_handle);
    return rc;
}
...
ble_att_svr_rx_read(uint16_t conn_handle, uint16_t cid, struct os_mbuf **rxom)
{
    ...
    *rxom = NULL;
    ...
    rc = ble_att_svr_create_read_rsp(conn_handle, cid, txom, rc,
                                     BLE_ATT_OP_READ_REQ, err_handle, 0,
                                     &att_err);
...
ble_att_svr_rx_read_blob(uint16_t conn_handle, uint16_t cid, struct os_mbuf **rxom)
{
    ...
    *rxom = NULL;
    ...
    if (rc == BLE_HS_EPENDING) {
        pending_att_prepare(conn_handle, err_handle, 0, txom,
                            BLE_ATT_ACCESS_OP_READ, BLE_ATT_OP_READ_BLOB_REQ,
                            cid);
        txom = NULL;
        return rc;
    }
                                 ...
                                 uint8_t *att_err, uint16_t *err_handle)
{
    ...
    rc = ble_att_svr_tx_rsp(conn_handle, cid, rc, txom,
                            BLE_ATT_OP_READ_MULT_REQ, *att_err,
                            *err_handle);
    return rc;
}
...
ble_att_svr_rx_read_mult(uint16_t conn_handle, uint16_t cid, struct os_mbuf **rxom)
{
    ...
    while ((OS_MBUF_PKTLEN(*rxom) - offset) >= 2) {
        ...
        if (rc == BLE_HS_EPENDING) {
            pending_att_prepare(conn_handle, 0, offset, *rxom,
    ...
    rc = ble_att_svr_create_read_mult_rsp(conn_handle, cid, rxom, rc,
                                          &att_err, &err_handle);
                                         ...
                                         uint16_t *err_handle)
{
    ...
    rc = ble_att_svr_tx_rsp(conn_handle, cid, rc, txom,
                            BLE_ATT_OP_READ_MULT_VAR_REQ, *att_err,
                            *err_handle);
    return rc;
}
...
ble_att_svr_rx_read_mult_var(uint16_t conn_handle, uint16_t cid, struct os_mbuf **rxom)
{
    ...
    return ble_att_svr_create_read_mult_var_len_rsp(conn_handle, cid, rxom,
                                                    rc, &att_err,
                                                    &err_handle);
}
                             ...
                             uint16_t handle, uint8_t *out_att_err)
{
    ...
    rc = ble_att_svr_write_handle(conn_handle, handle,
                                  offset, om, out_att_err);
...
ble_att_svr_rx_prep_write(uint16_t conn_handle, uint16_t cid, struct os_mbuf **rxom)
{
    ...
    if (rc == BLE_HS_EPENDING) {
        pending_att_prepare(conn_handle, err_handle, 0, *rxom,
                            BLE_ATT_ACCESS_OP_WRITE, BLE_ATT_OP_PREP_WRITE_REQ,
                            cid);
        /* Always reuse rxom mbuf for response */
        *rxom = NULL;
        return rc;
