...
ble_eatt_l2cap_event_fn(struct ble_l2cap_event *event, void *arg)
{
    ...
    if (event->type == BLE_L2CAP_EVENT_COC_CONNECTED) {
        ...
        if (req && req->magic == BLE_EATT_CONN_REQ_MAGIC &&
            req->conn_handle == event->connect.conn_handle) {
...
{
    struct os_mbuf *om[MYNEWT_VAL(BLE_EATT_CHAN_PER_CONN) > 0 ? MYNEWT_VAL(BLE_EATT_CHAN_PER_CONN) : 1];
    struct ble_eatt_conn_req *req;
    uint8_t channels;
    int rc;
    int i;
...
ble_eatt_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        while ((eatt = ble_eatt_find_by_conn_handle(
                    event->disconnect.conn.conn_handle)) != NULL) {
                ble_eatt_free(eatt);
        }
        break;
    default:
