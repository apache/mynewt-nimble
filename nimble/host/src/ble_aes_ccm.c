...
static inline void
xor16(uint8_t *dst, const uint8_t *a, const uint8_t *b)
{
    dst[0] = a[0] ^ b[0];
    dst[1] = a[1] ^ b[1];
    dst[2] = a[2] ^ b[2];
...
static int
ble_aes_ccm_calculate_X0(const uint8_t key[16], const uint8_t *aad, uint8_t aad_len,
...
static int
ble_aes_ccm_auth(const uint8_t key[16], uint8_t nonce[13],
...
static int
ble_aes_ccm_crypt(const uint8_t key[16], const uint8_t nonce[13],
...
int
ble_aes_ccm_decrypt(const uint8_t key[16], uint8_t nonce[13], const uint8_t *enc_msg,
...
int
ble_aes_ccm_encrypt(const uint8_t key[16], uint8_t nonce[13], const uint8_t *msg,
                    size_t msg_len, const uint8_t *aad, size_t aad_len,
                    uint8_t *out_msg, size_t mic_size)
{
