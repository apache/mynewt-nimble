...
static int
ble_ead_rand(void *buf, size_t len)
{
    int rc;
...
static int
ead_encrypt(const uint8_t session_key[BLE_EAD_KEY_SIZE], const uint8_t iv[BLE_EAD_IV_SIZE],
...
int
ble_ead_encrypt(const uint8_t session_key[BLE_EAD_KEY_SIZE], const uint8_t iv[BLE_EAD_IV_SIZE],
...
static int
ead_decrypt(const uint8_t session_key[BLE_EAD_KEY_SIZE], const uint8_t iv[BLE_EAD_IV_SIZE],
...
int
ble_ead_decrypt(const uint8_t session_key[BLE_EAD_KEY_SIZE], const uint8_t iv[BLE_EAD_IV_SIZE],
                const uint8_t *encrypted_payload, size_t encrypted_payload_size,
                uint8_t *payload)
{
    if (session_key == NULL) {
...
int
ble_ead_serialize_data(const struct enc_adv_data *input, uint8_t *output)
{
    if (input == NULL) {
