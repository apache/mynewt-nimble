...
struct ble_cs_sm {
    ...
    struct ble_cs_subevent_info local_subevent_info[BLE_HS_CS_SUBEVENT_COUNT_MAX];
    struct ble_cs_subevent_info remote_subevent_info[BLE_HS_CS_SUBEVENT_COUNT_MAX];
    uint8_t current_local_subevent_info_id;
    uint8_t current_remote_subevent_info_id;
};
...
static struct ble_gap_event_listener ble_cs_gap_listener;

static int ble_cs_create_config_mode2_test(struct ble_cs_sm *cssm);
...
ble_cs_call_procedure_complete_cb(struct ble_cs_sm *cssm, uint8_t status)
{
    ...
    cssm->current_local_subevent_info_id = 0;
    cssm->current_remote_subevent_info_id = 0;
}

static void
ble_cs_call_subevent_data_cb(struct ble_cs_sm *cssm, uint8_t status, uint8_t subevent_id)
{
    struct ble_cs_event event;

    assert(subevent_id < ARRAY_SIZE(cssm->local_subevent_info));
    assert(subevent_id < ARRAY_SIZE(cssm->remote_subevent_info));

    memset(&event, 0, sizeof event);
    event.type = BLE_CS_EVENT_CS_SUBEVENT_DATA;
    event.subevent_data.status = status;
    event.subevent_data.local_subevent_info = &cssm->local_subevent_info[subevent_id];
...
#if MYNEWT_VAL(BLE_CHANNEL_SOUNDING_FAE_SUPPORTED)
...
#else
    ble_cs_create_config_mode2_test(cssm);
#endif
                                         ...
                                         unsigned int len)
{
    ...
    rc = ble_cs_create_config_mode2_test(cssm);

    return rc;
}

static int
ble_cs_create_config_mode2_test(struct ble_cs_sm *cssm)
{
    int rc;
    ...
    memcpy(cmd.channel_map, (uint8_t[10]){
        0xFC, 0xFF, 0x7F, 0xFC, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x1F }, 10);
    cmd.channel_map_repetition = 0x01;
    cmd.channel_selection_type = BLE_HOST_CS_CSA_TYPE_3B;
    cmd.ch3c_shape = BLE_HOST_CS_CH3C_SHAPE_HAT;
    cmd.ch3c_jump = 0x00;
                                     ...
                                     unsigned int len)
{
    ...
    if (rc) {
        BLE_HS_LOG(DEBUG, "Failed to enable CS security");
        ble_cs_call_procedure_complete_cb(cssm, BLE_CS_EVENT_CS_PROCEDURE_ERR_SETUP_FAILED);
    }
                        ...
                        uint8_t data_len, uint8_t channel)
{
    ...
    assert(cssm->current_local_subevent_info_id < ARRAY_SIZE(cssm->local_subevent_info));
    subevent_info = &cssm->local_subevent_info[cssm->current_local_subevent_info_id];

    result = &subevent_info->mode0_results[subevent_info->mode0_results_count];
                        ...
                        uint8_t data_len, uint8_t channel)
{
    ...
    result = &subevent_info->mode1_results[subevent_info->mode1_results_count];
    memset(result, 0, sizeof(*result));
    result->step_channel = channel;
    result->packet_quality = data[0];
                        ...
                        uint8_t data_len, uint8_t channel)
{
    ...
    result = &subevent_info->mode2_results[subevent_info->mode2_results_count];
    memset(result, 0, sizeof(*result));
    result->step_channel = channel;
                        ...
                        uint8_t data_len, uint8_t channel)
{
    ...
    result = &subevent_info->mode3_results[subevent_info->mode3_results_count];
    memset(result, 0, sizeof(*result));
    result->step_channel = channel;
    result->packet_quality = data[0];
    result->packet_nadm = data[1];
...
static int
ble_cs_add_local_steps_results(struct ble_cs_sm *cssm, const struct cs_steps_data *step_data,
                               uint8_t step_count)
{
    int rc = 1;
    const void *data;
    ...
    for (i = 0; i < step_count; ++i) {
        ...
        ++cssm->local_subevent_info[cssm->current_local_subevent_info_id].num_steps_reported;
                                     ...
                                     unsigned int len)
{
    ...
    if (ev->subevent_done_status == BLE_HS_CS_SUBEVENT_DONE_STATUS_COMPLETED) {
        ++cssm->current_local_subevent_info_id;
    }
...
static void
ble_cs_on_disconnect(uint16_t conn_handle)
{
    ...
    cssm->conn_handle = BLE_CONN_HANDLE_INVALID;
}

static int
ble_cs_gap_event(struct ble_gap_event *event, void *arg)
{
    switch (event->type) {
    case BLE_GAP_EVENT_DISCONNECT:
        ble_cs_on_disconnect(event->disconnect.conn.conn_handle);
        break;
    default:
        break;
    }
...
ble_cs_init(void)
{
    ...
    rc = ble_gap_event_listener_register(&ble_cs_gap_listener, ble_cs_gap_event, NULL);
    assert(rc == 0);
                       ...
                       uint16_t conn_handle)
{
    ...
    memset(&cssm->local_subevent_info, 0, sizeof(cssm->local_subevent_info));
    memset(&cssm->remote_subevent_info, 0, sizeof(cssm->remote_subevent_info));
    cssm->current_local_subevent_info_id = 0;
    cssm->current_remote_subevent_info_id = 0;
...
ble_cs_setup(const struct ble_cs_setup_params *params, uint16_t conn_handle)
{
    ...
    config->rtt_type = BLE_HS_CS_RTT_AA_ONLY;
