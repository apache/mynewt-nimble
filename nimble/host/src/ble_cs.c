...
static uint8_t aci_to_antenna_paths_mask[] = {0b1, 0b11, 0b111, 0b1111, 0b11, 0b111, 0b1111, 0b1111};

static struct ble_gap_event_listener ble_cs_listener;
...
ble_cs_call_event_cb(struct ble_cs_sm *cssm, struct ble_cs_event *event)
{
    ...
    if (cssm->event_cb != NULL) {
        rc = cssm->event_cb(event, cssm->event_cb_arg);
    } else {
        rc = 0;
    }
...
ble_cs_call_procedure_complete_cb(struct ble_cs_sm *cssm, uint8_t status)
{
    ...
    event.conn_handle = cssm->conn_handle;
    event.status = status;
...
static void
ble_cs_call_step_data_cb(struct ble_cs_sm *cssm, void *step_data, uint8_t status,
                         uint8_t step_mode, uint8_t role)
{
    struct ble_cs_event event;
...
ble_cs_step_data_received_cb(void *data, uint16_t conn_handle, uint8_t step_mode)
{
    ...
    remote_role = config->local_role == BLE_HS_CS_ROLE_INITIATOR ?
                  BLE_HS_CS_ROLE_REFLECTOR : BLE_HS_CS_ROLE_INITIATOR;
                                         ...
                                         unsigned int len)
{
    ...
    if (len != sizeof(*ev) || (ev->status != BLE_ERR_SUCCESS &&
                               ev->status != BLE_ERR_UNSUPPORTED)) {
        return BLE_HS_ECONTROLLER;
    }
...
volatile static int retry_counter = 10;
                                          ...
                                          unsigned int len)
{
    ...
    ble_svc_ras_ranging_data_body_init(cssm->conn_handle, ev->procedure_count, ev->config_id,
                                       ev->selected_tx_power, antenna_paths_mask);
#endif
...
static int
ble_cs_add_mode0_result(struct ble_cs_sm *cssm, const uint8_t *data, uint8_t data_len)
{
...
#else
    ble_cs_call_step_data_cb(cssm, (void *)&result, 0, BLE_HS_CS_MODE0, config->local_role);
#endif
...
static int
ble_cs_add_mode1_result(struct ble_cs_sm *cssm, const uint8_t *data, uint8_t data_len)
{
...
#else
    ble_cs_call_step_data_cb(cssm, (void *)&result, 0, BLE_HS_CS_MODE1, config->local_role);
#endif
...
static int
ble_cs_add_mode2_result(struct ble_cs_sm *cssm, const uint8_t *data, uint8_t data_len)
{
    ...
    memcpy(result.antenna_paths, antenna_path_permutations[result.antenna_path_permutation_id],
...
#else
    ble_cs_call_step_data_cb(cssm, (void *)&result, 0, BLE_HS_CS_MODE2, config->local_role);
#endif
...
static int
ble_cs_add_mode3_result(struct ble_cs_sm *cssm, const uint8_t *data_buf, uint8_t data_len)
{
    const uint8_t *data = data_buf;
    struct ble_cs_mode3_result result;
    const uint8_t *antenna_paths;
    struct ble_cs_config *config = &cssm->config[cssm->active_config_id];
...
#if MYNEWT_VAL(BLE_SVC_RAS_SERVER)
    ble_svc_ras_add_mode3_result(&result, cssm->conn_handle, n_ap, config->rtt_pct_included);
#else
    ble_cs_call_step_data_cb(cssm, (void *)&result, 0, BLE_HS_CS_MODE3, config->local_role);
#endif
                 ...
                 uint8_t step_count)
{
    ...
    for (i = 0; i < step_count; ++i) {
        ...
        case BLE_HS_CS_MODE0:
            rc = ble_cs_add_mode0_result(cssm, data, data_len);
            break;
        case BLE_HS_CS_MODE1:
            rc = ble_cs_add_mode1_result(cssm, data, data_len);
            break;
        case BLE_HS_CS_MODE2:
            rc = ble_cs_add_mode2_result(cssm, data, data_len);
            break;
        case BLE_HS_CS_MODE3:
            rc = ble_cs_add_mode3_result(cssm, data, data_len);
            break;
        default:
            rc = 1;
        }
                                     ...
                                     unsigned int len)
{
    ...
    ble_svc_ras_ranging_subevent_init(cssm->conn_handle,
                                      le16toh(ev->start_acl_conn_event_counter),
                                      le16toh(ev->frequency_compensation),
                                      ev->procedure_done_status,
                                      ev->subevent_done_status,
                                      ranging_abort_reason,
                                      subevent_abort_reason,
                                      ev->reference_power_level,
                                              ...
                                              unsigned int len)
{
    ...
    ble_svc_ras_ranging_subevent_update_status(cssm->conn_handle,
                                               ev->num_steps_reported,
                                               ev->procedure_done_status,
                                               ev->subevent_done_status,
                                               ranging_abort_reason,
...
ble_cs_procedure_start(const struct ble_cs_procedure_start_params *params)
{
    ...
    cssm = ble_cs_sm_get(params->conn_handle);
    assert(cssm != NULL);
    cssm->pending_procedure = 1;

#if MYNEWT_VAL(BLE_SVC_RAS_CLIENT)
    return ble_svc_ras_clt_subscribe(ble_cs_initiator_config_cb, ble_cs_step_data_received_cb,
...
int
ble_cs_procedure_terminate(uint16_t conn_handle)
{
    return 0;
}

int
ble_cs_setup(struct ble_cs_setup_params *params)
{
    struct ble_cs_sm *cssm = NULL;
    struct ble_cs_config *config;
    ...
    cssm->conn_handle = params->conn_handle;
    ...
    config = &cssm->config[cssm->active_config_id];
#if MYNEWT_VAL(BLE_SVC_RAS_CLIENT)
    config->local_role = BLE_HS_CS_ROLE_REFLECTOR;
//    config->local_role = BLE_HS_CS_ROLE_INITIATOR;
#else
    config->local_role = BLE_HS_CS_ROLE_REFLECTOR;
#endif
