...
static SLIST_HEAD(, ble_peer) peers;

static struct ble_peer_svc *peer_svc_find_range(struct ble_peer *peer, uint16_t attr_handle);
static struct ble_peer_svc *peer_svc_find(struct ble_peer *peer, uint16_t svc_start_handle, struct ble_peer_svc **out_prev);
int ble_peer_svc_is_empty(const struct ble_peer_svc *svc);
uint16_t ble_peer_chr_end_handle(const struct ble_peer_svc *svc, const struct ble_peer_chr *chr);
int ble_peer_chr_is_empty(const struct ble_peer_svc *svc, const struct ble_peer_chr *chr);
static struct ble_peer_chr *peer_chr_find(const struct ble_peer_svc *svc, uint16_t chr_def_handle, struct ble_peer_chr **out_prev);
...
peer_disc_dscs(struct ble_peer *peer)
{
    ...
    SLIST_FOREACH(svc, &peer->svcs, next) {
        SLIST_FOREACH(chr, &svc->chrs, next) {
            if (!ble_peer_chr_is_empty(svc, chr) &&
