...
ble_gatts_bonding_restored(uint16_t conn_handle)
{
    ...
    awareness = conn->bhc_gatt_svr.awareness;
    ...
    if (!ble_store_read_peer_cl_sup_feat(&feat_key, &feat_val)) {
        ...
        if ((feat_val.peer_cl_sup_feat[0] & (1 << 0)) != 0) {
            if (awareness == BLE_GATTS_HASH_READ_PENDING) {
                /* Peer read the hash over an unencrypted link right before
                 * bonding completed. Store it now. */
                db_hash_store(conn_handle);
            } else if (ble_store_read_db_hash(&db_hash_key, &db_hash_value) ==
                       BLE_HS_ENOENT) {
                /* Peer with no historical hash record. Force
                 * them unaware to guarantee a clean sync cycle. */
                set_change_unaware(conn_handle, NULL);
            } else {
                /* Hash record exists - compare it against the active server layout */
...
is_change_aware(uint16_t conn_handle)
{
    ...
    /* If peer doesn't support Robust Caching, they are always change aware */
    ...
    if (conn->bhc_gatt_svr.awareness == BLE_GATTS_HASH_READ_PENDING) {
        conn->bhc_gatt_svr.awareness = BLE_GATTS_CHANGE_AWARE;
        status = true;
        goto done;
    }

    if (conn->bhc_gatt_svr.awareness == BLE_GATTS_OUT_OF_SYNC_SENT) {
        /* TODO: set BLE_GATTS_CHANGE_AWARE state only for clients using
         * exactly one ATT bearer. */
        conn->bhc_gatt_svr.awareness = BLE_GATTS_CHANGE_AWARE;
        status = true;
        goto done;
    }
...
db_hash_store(uint16_t conn_handle)
{
    ...
    global_hash = ble_gatts_get_db_hash();
    if (global_hash != NULL) {
        /* Copies the raw 16 bytes directly into the flat structure */
        memcpy(store_hash.db_hash, global_hash, 16);
    } else {
        memset(store_hash.db_hash, 0, 16);
    }
