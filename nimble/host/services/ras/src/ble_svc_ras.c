...
int
ble_svc_ras_ranging_data_body_init(uint16_t conn_handle, uint16_t procedure_counter,
                                   uint8_t config_id, uint8_t tx_power, uint8_t antenna_paths_mask)
{
...
int
ble_svc_ras_ranging_subevent_init(uint16_t conn_handle,
...
int
ble_svc_ras_add_mode0_result(struct ble_cs_mode0_result *result, uint16_t conn_handle, uint8_t local_role)
{
    int rc;
    struct ble_svc_ras_sm *rassm;
    struct os_mbuf *om;
...
int
ble_svc_ras_add_mode1_result(struct ble_cs_mode1_result *result, uint16_t conn_handle, uint8_t rtt_pct_included)
{
    struct ble_svc_ras_sm *rassm;
    struct os_mbuf *om;
    struct os_mbuf *om2;
...
int
ble_svc_ras_add_mode2_result(struct ble_cs_mode2_result *result, uint16_t conn_handle, uint8_t n_ap)
{
    struct ble_svc_ras_sm *rassm;
    struct os_mbuf *om;
    struct os_mbuf *om2;
...
static int
ble_svc_ras_ranging_data_notify(struct ble_svc_ras_sm *rassm, uint8_t notify, uint8_t indicate)
{
    int rc;
    int len;
    struct os_mbuf *om;
...
ble_svc_ras_cp_rsp_complete_ranging_data_notify(struct ble_svc_ras_sm *rassm)
{
    ...
    rc = ble_gatts_indicate_custom(rassm->conn_handle, ble_svc_ras_control_point_val_handle, om);

    return rc;
}

static int
ble_svc_ras_cp_rsp_complete_lost_segment_notify(struct ble_svc_ras_sm *rassm, uint16_t ranging_counter,
                                                uint8_t first_segment_id, uint8_t last_segment_id)
{
    int rc;
    struct os_mbuf *om;
    uint8_t *buf;
...
static int
ras_cp_cmd_get_ranging_data(struct ble_svc_ras_sm *rassm, const struct os_mbuf *om, int off)
{
    int rc;
    uint16_t ranging_counter;
...
static int
ras_cp_cmd_ack_ranging_data(struct ble_svc_ras_sm *rassm, const struct os_mbuf *om, int off)
{
    int rc;
    uint16_t ranging_counter;
...
static int
ras_cp_cmd_retrieve_lost_segment(struct ble_svc_ras_sm *rassm, const struct os_mbuf *om, int off)
{
    int rc;
    uint8_t buf[4];
    uint16_t ranging_counter;
...
ble_svc_ras_ranging_data_ready_notify(struct ble_svc_ras_sm *rassm)
{
    ...
    if (rassm->notify & BIT(NOTIFY_RANGING_DATA_READY)) {
        rc = ble_gatts_notify_custom(rassm->conn_handle,
                                     ble_svc_ras_ranging_data_ready_val_handle, om);
    } else {
...
static int
ble_svc_ras_ranging_data_overwritten_notify(struct ble_svc_ras_sm *rassm, uint32_t toa_tod_val)
{
    int rc;
    struct os_mbuf *om;
    uint8_t *buf;
    ...
    if (rassm->notify & BIT(NOTIFY_RANGING_DATA_OVERWRITTEN)) {
        rc = ble_gatts_notify_custom(rassm->conn_handle,
                                     ble_svc_ras_ranging_data_overwritten_val_handle, om);
    } else {
        rc = ble_gatts_indicate_custom(rassm->conn_handle,
                                       ble_svc_ras_ranging_data_overwritten_val_handle, om);
    }
...
ble_svc_ras_on_subscribe(struct ble_gap_event *event)
{
    ...
    if (event->subscribe.attr_handle == ble_svc_ras_real_time_ranging_data_val_handle) {
        if (rassm->notify & BIT(NOTIFY_ON_DEMAND_RANGING_DATA) ||
            ...
            /* The RAS Server shall operate in either Real-time or On-demand mode,
        ...
        notify = NOTIFY_REAL_TIME_RANGING_DATA;
        indicate = INDICATE_REAL_TIME_RANGING_DATA;
    } else if (event->subscribe.attr_handle == ble_svc_ras_on_demand_ranging_data_val_handle) {
        if (rassm->notify & BIT(NOTIFY_REAL_TIME_RANGING_DATA) ||
            rassm->indicate & BIT(INDICATE_REAL_TIME_RANGING_DATA)) {
            return BLE_ATT_ERR_CCCD_IMPORER_CONF;
        }
    ...
    } else if (event->subscribe.attr_handle == ble_svc_ras_ranging_data_overwritten_val_handle) {
...
ble_svc_ras_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    switch (event->type) {
        ...
        case BLE_GAP_EVENT_SUBSCRIBE:
             ...
             * but for now API does not allow that.
             */
            break;
...
ble_svc_ras_init(void)
{
    ...
    rc = os_mempool_init(&ras_mempool, MBUF_POOL_BUF_COUNT,
            MBUF_POOL_BUF_SIZE, &ras_mbuf_area[0], "ras_mbuf_pool");
    assert(rc == 0);

    rc = os_mbuf_pool_init(&ras_mbuf_pool, &ras_mempool,
    ...
    rc = ble_gap_event_listener_register(&ble_svc_ras_listener, ble_svc_ras_gap_event, NULL);
    assert(rc == 0);
}
