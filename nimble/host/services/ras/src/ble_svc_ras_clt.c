...
static int
ble_svc_ras_clt_mode0_parse(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om, int *off)
{
    int rc;
    ...
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE0_PACKET_QUALITY)) len += 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE0_PACKET_RSSI)) len += 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE0_PACKET_ANTENNA)) len += 1;
    if ((filter & BIT(BLE_SVC_RAS_FILTER_MODE0_MEASURED_FREQ_OFFSET)) && remote_initiator) len += 2;
...
static int
ble_svc_ras_clt_mode1_parse(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om, int *off)
{
    int rc;
    uint8_t *buf = subevent_buf;
    ...
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE1_PACKET_QUALITY)) len += 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE1_PACKET_NADM)) len += 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE1_PACKET_RSSI)) len += 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE1_TOD_TOA)) len += 2;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE1_PACKET_ANTENNA)) len += 1;
    if (rassm->rtt_pct_included) {
        if (filter & BIT(BLE_SVC_RAS_FILTER_MODE1_PACKET_PCT1)) len += 4;
        if (filter & BIT(BLE_SVC_RAS_FILTER_MODE1_PACKET_PCT2)) len += 4;
    }
...
static int
ble_svc_ras_clt_mode2_parse(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om, int *off)
{
    int rc;
    uint8_t *buf = subevent_buf;
    ...
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE2_ANTENNA_PERMUTATION_ID)) len += 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE2_TONE_PCT)) len += 3 * (n_ap + 1);
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE2_TONE_QUALITY)) len += n_ap + 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE2_ANTENNA_PATH_1)) len += 1;
    if (n_ap > 1 && (filter & BIT(BLE_SVC_RAS_FILTER_MODE2_ANTENNA_PATH_2))) len += 1;
    if (n_ap > 2 && (filter & BIT(BLE_SVC_RAS_FILTER_MODE2_ANTENNA_PATH_3))) len += 1;
    if (n_ap > 3 && (filter & BIT(BLE_SVC_RAS_FILTER_MODE2_ANTENNA_PATH_4))) len += 1;
...
static int
ble_svc_ras_clt_mode3_parse(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om, int *off)
{
    int rc;
    uint8_t *buf = subevent_buf;
    ...
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_PACKET_QUALITY)) len += 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_PACKET_NADM)) len += 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_PACKET_RSSI)) len += 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_TOD_TOA)) len += 2;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_PACKET_ANTENNA)) len += 1;
    if (rassm->rtt_pct_included) {
        if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_PACKET_PCT1)) len += 4;
        if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_PACKET_PCT2)) len += 4;
    }
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_ANTENNA_PERMUTATION_ID)) len += 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_TONE_PCT)) len += 3 * (n_ap + 1);
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_TONE_QUALITY)) len += n_ap + 1;
    if (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_ANTENNA_PATH_1)) len += 1;
    if (n_ap > 1 && (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_ANTENNA_PATH_2))) len += 1;
    if (n_ap > 2 && (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_ANTENNA_PATH_3))) len += 1;
    if (n_ap > 3 && (filter & BIT(BLE_SVC_RAS_FILTER_MODE3_ANTENNA_PATH_4))) len += 1;
...
static int
ble_svc_ras_clt_subevent_data_parse(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om, int *off,
...
static int
ble_svc_ras_clt_ranging_data_parse(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om,
                                   struct ble_svc_ras_clt_ranging_header *header)
{
    ...
    while (off < buf_len) {
        ...
        rc = ble_svc_ras_clt_subevent_data_parse(rassm, om, &off, sub_header.number_of_steps_reported);
        if (rc) {
            return rc;
        }
    }
...
static int
ble_svc_ras_clt_cp_write_flat(uint16_t conn_handle, const void *data, uint16_t data_len,
                              ble_gatt_attr_fn *cb, void *cb_arg)
{
    ...
    chr = ble_peer_chr_find_uuid(peer, BLE_UUID16_DECLARE(BLE_SVC_RAS_SVC_RANGING_SERVICE_UUID),
                                 BLE_UUID16_DECLARE(BLE_SVC_RAS_CHR_RAS_CONTROL_POINT_UUID));

    rc = ble_gattc_write_flat(conn_handle, chr->chr.val_handle, data, data_len, cb, cb_arg);

    return rc;
}

static int
ble_svc_ras_clt_cp_get_ranging_data(struct ble_svc_ras_clt_sm *rassm, uint16_t ranging_counter)
{
    uint8_t value[3];
    ...
    return ble_svc_ras_clt_cp_write_flat(rassm->conn_handle, value, sizeof(value), NULL, NULL);
}

static int
ble_svc_ras_clt_cp_ack_ranging_data(struct ble_svc_ras_clt_sm *rassm, uint16_t ranging_counter)
{
    uint8_t value[3];
...
ble_svc_ras_clt_cp_ack_retrieve_lost_segment(struct ble_svc_ras_clt_sm *rassm,
                                             uint16_t ranging_counter, uint8_t first_segment_id,
...
static int
ble_svc_ras_clt_cp_abort_operation(struct ble_svc_ras_clt_sm *rassm, uint16_t ranging_counter)
{
...
static int
ble_svc_ras_clt_cp_set_filter(struct ble_svc_ras_clt_sm *rassm, uint8_t mode, uint8_t filter)
{
    uint8_t value[2];
...
static int
ble_svc_ras_clt_cp_on_ranging_data_ready(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om)
{
    int rc;
    uint16_t ranging_counter;
...
ble_svc_ras_clt_on_notify_rx(struct ble_gap_event *event)
{
    ...
    if (attr_handle == rassm->ranging_data_ready_val_handle) {
        ...
        ble_svc_ras_clt_notification_parse(event->notify_rx.om, event->notify_rx.conn_handle);
                            ...
                            uint16_t svc_uuid, uint16_t chr_uuid)
{
    ...
    dsc = ble_peer_dsc_find_uuid(peer, BLE_UUID16_DECLARE(svc_uuid), BLE_UUID16_DECLARE(chr_uuid),
                                 BLE_UUID16_DECLARE(BLE_GATT_DSC_CLT_CFG_UUID16));

    value[0] = 0b10;
    value[1] = 0;
    rc = ble_gattc_write_flat(rassm->conn_handle, dsc->dsc.handle, value, sizeof(value), cb, rassm);

    return rc;
}

static int
ble_cs_subscribe_control_point_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
...
static int
ble_cs_subscribe_ranging_data_overwritten_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
                                             struct ble_gatt_attr *attr, void *arg)
{
...
static int
ble_cs_subscribe_ranging_data_ready_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
                                       struct ble_gatt_attr *attr, void *arg)
{
...
static int
ble_cs_subscribe_on_demand_ranging_data_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
                                           struct ble_gatt_attr *attr, void *arg)
{
...
static int
ble_cs_subscribe_real_time_ranging_data_cb(uint16_t conn_handle, const struct ble_gatt_error *error,
                                           struct ble_gatt_attr *attr, void *arg)
{
    return ble_cs_subscribe_indication(arg, ble_cs_subscribe_control_point_cb,
                                       BLE_SVC_RAS_SVC_RANGING_SERVICE_UUID,
                          ...
                          uint16_t conn_handle, uint8_t mode)
{
    ...
    return ble_cs_subscribe_indication(rassm, cb, BLE_SVC_RAS_SVC_RANGING_SERVICE_UUID, chr_uuid);
}
...
ble_svc_ras_clt_init(void)
{
    ...
    rc = os_mempool_init(&ras_clt_mempool, MBUF_POOL_BUF_COUNT,
            MBUF_POOL_BUF_SIZE, &ras_clt_mbuf_area[0], "ras_clt_mbuf_pool");
    assert(rc == 0);
    ...
    rc = ble_gap_event_listener_register(&ble_svc_ras_clt_listener, ble_svc_ras_clt_gap_event, NULL);
    assert(rc == 0);
}
