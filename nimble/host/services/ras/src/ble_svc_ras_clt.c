...
struct ble_svc_ras_clt_sm {
    ...
    ble_svc_ras_clt_ranging_header_received_cb *ranging_cb;
    ble_svc_ras_clt_subevent_header_received_cb *subevent_cb;
    ble_svc_ras_clt_step_data_received_cb *step_data_cb;
    ble_svc_ras_clt_ranging_data_complete_cb *ranging_data_complete_cb;
    ...
    uint16_t next_expected_ranging_counter;
...
static void
ble_ras_clt_flush(struct ble_svc_ras_clt_sm *rassm)
{
                            ...
                            struct os_mbuf *om, int *off)
{
    ...
    struct ble_cs_mode0_result result = {0};
    uint8_t len = 0;
    uint8_t i = 0;
                            ...
                            struct os_mbuf *om, int *off)
{
    int rc;
    uint8_t *buf = subevent_buf;
    struct ble_cs_mode1_result result = {0};
                            ...
                            struct os_mbuf *om, int *off)
{
    int rc;
    uint8_t *buf = subevent_buf;
    struct ble_cs_mode2_result result = {0};
    ...
    *off += len;
    ...
    /* TODO: Add support for filtering multiple antenna paths */
    assert(result.antenna_path_permutation_id == 0);
                            ...
                            struct os_mbuf *om, int *off)
{
    int rc;
    uint8_t *buf = subevent_buf;
    struct ble_cs_mode3_result result = {0};
...
ble_svc_ras_clt_ranging_data_parse(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om)
{
    ...
    rc = ble_svc_ras_clt_ranging_header_parse(&ranging_header, om, &off);
    if (rc) {
        return rc;
    }

    rassm->ranging_cb(&ranging_header, rassm->conn_handle);

    while (off < buf_len) {
        ble_svc_ras_clt_subevent_parse(&sub_header, om, &off);
        rassm->subevent_cb(&sub_header, rassm->conn_handle);
...
ble_svc_ras_clt_notification_parse(struct os_mbuf *src_om, uint16_t conn_handle)
{
    ...
    if (is_last_segment) {
        ble_svc_ras_clt_ranging_data_parse(rassm, cache_om);
        rassm->ranging_data_complete_cb(conn_handle);
    }
...
static int
ble_svc_ras_clt_cp_ack_ranging_data(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om)
{
    int rc;
    uint16_t ranging_counter;
    uint8_t value[3];

    rc = os_mbuf_copydata(om, 1, sizeof(ranging_counter), &ranging_counter);
    if (rc != 0) {
        return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;
    }

    ranging_counter = get_le16(&ranging_counter);
    if (ranging_counter != rassm->ranging_counter) {
        /* Ignore an unexpected ranging counter */
        return 0;
    }
...
static int
ble_svc_ras_clt_cp_complete_lost_segment(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om)
{
    int rc;
    uint8_t buf[4];
    uint16_t ranging_counter;
    uint8_t first_lost_seg;
    uint8_t last_lost_seg;

    rc = os_mbuf_copydata(om, 1, sizeof(buf), buf);
    if (rc != 0) {
        return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;
    }

    ranging_counter = get_le16(buf);
    BLE_HS_LOG(DEBUG, "RAS: Received Complete Lost Segment response, ranging_counter %u", ranging_counter);

    /* TODO: Handle these */
    first_lost_seg = buf[2];
    last_lost_seg = buf[3];
    (void)first_lost_seg;
    (void)last_lost_seg;
...
static int
ble_svc_ras_clt_cp_response_code_handle(struct ble_svc_ras_clt_sm *rassm, struct os_mbuf *om)
{
    int rc;
    uint8_t response_code;

    rc = os_mbuf_copydata(om, 1, sizeof(response_code), &response_code);
    if (rc != 0) {
        return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;
    }

    BLE_HS_LOG(DEBUG, "RAS: Response Code %u", response_code);

    /* TODO: Handle these */
    switch (response_code) {
    case BLE_SVC_RAS_CP_RSPCODE_SUCCESS:
        break;
    case BLE_SVC_RAS_CP_RSPCODE_OP_CODE_NOT_SUPPORTED:
        break;
    case BLE_SVC_RAS_CP_RSPCODE_INVALID_PARAMETER:
        break;
    case BLE_SVC_RAS_CP_RSPCODE_SUCCESS_PERSISTED:
        break;
    case BLE_SVC_RAS_CP_RSPCODE_ABORT_UNSUCCESSFUL:
        break;
    case BLE_SVC_RAS_CP_RSPCODE_PROCEDURE_NOT_COMPLETED:
        break;
    case BLE_SVC_RAS_CP_RSPCODE_SERVER_BUSY:
        break;
    case BLE_SVC_RAS_CP_RSPCODE_NO_RECORDS_FOUND:
        break;
    default:
                                         ...
                                         struct os_mbuf *om)
{
    ...
    ranging_counter = get_le16(&ranging_counter);
    if (ranging_counter < rassm->next_expected_ranging_counter) {
        /* Ignore duplicated Ranging Data Ready/Overwritten indications */
        BLE_HS_LOG(DEBUG, "RAS: Received unexpected ranging counter: ranging_counter %u, "
                          "expected %u", ranging_counter, rassm->next_expected_ranging_counter);
        return 0;
    }

    rassm->next_expected_ranging_counter++;
    rassm->ranging_counter = ranging_counter;

    return ble_svc_ras_clt_cp_get_ranging_data(rassm, rassm->ranging_counter);
}

static int
ble_svc_ras_clt_cp_on_ranging_data_overwritten(struct ble_svc_ras_clt_sm *rassm,
                                               struct os_mbuf *om)
{
    int rc;
    uint16_t ranging_counter;
    ...
    ranging_counter = get_le16(&ranging_counter) + 1;
    if (ranging_counter < rassm->next_expected_ranging_counter) {
        /* Ignore duplicated Ranging Data Ready/Overwritten indications */
        BLE_HS_LOG(DEBUG, "RAS: Received unexpected ranging counter: ranging_counter %u, "
                          "expected %u", ranging_counter, rassm->next_expected_ranging_counter);
...
static int
ble_svc_ras_clt_cp_on_control_point_notification(struct ble_svc_ras_clt_sm *rassm,
                                                 struct os_mbuf *om)
{
    int rc;
    uint8_t opcode;

    rc = os_mbuf_copydata(om, 0, sizeof(opcode), &opcode);
    if (rc != 0) {
        return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;
    }

    if (opcode == BLE_SVC_RAS_CP_RSP_COMPLETE_RANGING_DATA) {
        return ble_svc_ras_clt_cp_ack_ranging_data(rassm, om);
    } else if (opcode == BLE_SVC_RAS_CP_RSP_COMPLETE_LOST_SEGMENT) {
        return ble_svc_ras_clt_cp_complete_lost_segment(rassm, om);
    } else if (opcode == BLE_SVC_RAS_CP_RSP_RESPONSE_CODE) {
        return ble_svc_ras_clt_cp_response_code_handle(rassm, om);
    } else {
        return BLE_ATT_ERR_UNLIKELY;
    }
...
ble_svc_ras_clt_on_notify_rx(struct ble_gap_event *event)
{
    ...
    if (attr_handle == rassm->ranging_data_ready_val_handle) {
        ...
        ble_svc_ras_clt_cp_on_ranging_data_overwritten(rassm, event->notify_rx.om);
    } else if (attr_handle == rassm->control_point_val_handle) {
        ble_svc_ras_clt_cp_on_control_point_notification(rassm, event->notify_rx.om);
    }
}
                          ...
                          uint16_t conn_handle, uint8_t mode)
{
    ...
    rassm->ranging_cb = ranging_cb;
    rassm->subevent_cb = subevent_cb;
    rassm->step_data_cb = step_data_cb;
    rassm->ranging_data_complete_cb = ranging_data_complete_cb;
