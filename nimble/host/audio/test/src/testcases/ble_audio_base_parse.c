...
static const uint8_t example_base[] = {
    /* Presentation_Delay: 40 ms */
    0x1e, 0x00, 0x00,
    /* Num_Subgroups: 2 Subgroups */
    0x02,
    /* Num_BIS[0]: 2 BIS in Subgroup[0] */
    0x02,
    /* Codec_ID[0]: LC3 */
...
TEST_CASE_SELF(ble_audio_base_parse_test)
{
    ...
    TEST_ASSERT(subgroup.codec_specific_config_len == 10);
    TEST_ASSERT(subgroup.codec_specific_config != NULL);
    ...
    TEST_ASSERT(bis.codec_specific_config_len == 6);
    TEST_ASSERT(bis.codec_specific_config != NULL);
...
TEST_CASE_SELF(ble_audio_base_parse_test_length)
{
    struct ble_audio_base_subgroup subgroup;
    struct ble_audio_base_group group;
    struct ble_audio_base_bis bis;
...
TEST_SUITE(ble_audio_base_parse_test_suite)
{
    ...
    ble_audio_base_parse_test_length();
}
