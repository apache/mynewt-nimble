...
typedef int ble_svc_audio_bass_ctrl_point_handler_cb(uint8_t *data, uint16_t conn_handle);
...
static int
ble_svc_audio_bass_remote_scan_stopped(uint8_t *data, uint16_t conn_handle);
static int
ble_svc_audio_bass_remote_scan_started(uint8_t *data, uint16_t conn_handle);
static int
ble_svc_audio_bass_add_source(uint8_t *data, uint16_t conn_handle);
static int
ble_svc_audio_bass_modify_source(uint8_t *data, uint16_t conn_handle);
static int
...
{
    struct ble_audio_event ev = {
        .type = BLE_AUDIO_EVENT_BASS_ADD_SOURCE
    };
    ...
    if (operation.add_source.pa_sync > 0x02) {
        return BLE_HS_EINVAL;
    }

    operation.add_source.pa_interval = get_le16(&data[offset]);
    offset += 2;
    operation.add_source.num_subgroups = get_le16(&data[offset]);
    offset += 2;
    ...
    *operation.add_source.source_id = BLE_SVC_AUDIO_BASS_RECEIVE_STATE_SRC_ID_NONE;
    ...
    if (*operation.add_source.source_id == BLE_SVC_AUDIO_BASS_RECEIVE_STATE_SRC_ID_NONE) {
        ...
        ble_svc_audio_bass_receive_state_find_by_source_id(&rcv_state, *operation.add_source.source_id);
    }

    ev.bass_source_added.source_id = rcv_state->source_id;
    ...
    for (i = 0; i < operation.add_source.num_subgroups; i++) {
        ...
        memcpy(metadata_ptr, operation.add_source.subgroups[i].metadata,
               min(operation.add_source.subgroups[i].metadata_length,
                   MYNEWT_VAL(BLE_SVC_AUDIO_BASS_METADATA_MAX_SZ)));
        if (operation.add_source.subgroups[i].metadata_length > 0) {
            rcv_state->state.subgroups[i].metadata = metadata_ptr;
        }
        rcv_state->state.subgroups[i].metadata = operation.add_source.subgroups[i].metadata;
    }
...
ble_svc_audio_bass_modify_source(uint8_t *data, uint16_t conn_handle)
{
    ...
    struct ble_svc_audio_bass_rcv_state_entry *rcv_state = NULL;
    struct ble_audio_event ev = {
        .type = BLE_AUDIO_EVENT_BASS_MODIFY_SOURCE
    };
    uint8_t offset = 0;
    int rc;
    int i;
    ...
    ev.bass_source_modify.source_id = operation.modify_source.source_id;

    if (operation.modify_source.pa_sync == BLE_SVC_AUDIO_BASS_PA_SYNC_DO_NOT_SYNC &&
        rcv_state->state.pa_sync_state != BLE_SVC_AUDIO_BASS_PA_SYNC_STATE_SYNCED) {
        rcv_state->state.pa_sync_state = BLE_SVC_AUDIO_BASS_PA_SYNC_STATE_NOT_SYNCED;
        ev.bass_source_modify.pa_sync_operation = BLE_AUDIO_EVENT_BASS_NO_SYNC;
    } else if (operation.modify_source.pa_sync == BLE_SVC_AUDIO_BASS_PA_SYNC_DO_NOT_SYNC &&
               rcv_state->state.pa_sync_state == BLE_SVC_AUDIO_BASS_PA_SYNC_STATE_SYNCED) {
        rcv_state->state.pa_sync_state = BLE_SVC_AUDIO_BASS_PA_SYNC_STATE_NOT_SYNCED;
        ev.bass_source_modify.pa_sync_operation = BLE_AUDIO_EVENT_BASS_SYNC_TERMINATE;
    } else if (operation.modify_source.pa_sync == BLE_SVC_AUDIO_BASS_PA_SYNC_SYNC_PAST_AVAILABLE) {
        if (MYNEWT_VAL(BLE_PERIODIC_ADV_SYNC_TRANSFER)) {
            ev.bass_source_modify.pa_sync_operation = BLE_AUDIO_EVENT_BASS_SYNC_PAST;
            rcv_state->state.pa_sync_state = BLE_SVC_AUDIO_BASS_PA_SYNC_STATE_SYNC_INFO_REQ;
        } else {
            ev.bass_source_modify.pa_sync_operation = BLE_AUDIO_EVENT_BASS_SYNC;
        }
    } else if (operation.modify_source.pa_sync ==
...
ble_svc_audio_bass_set_broadcast_code(uint8_t *data, uint16_t conn_handle)
{
    struct ble_svc_audio_bass_rcv_state_entry *rcv_state = NULL;
    struct ble_audio_event ev = {
        .type = BLE_AUDIO_EVENT_BASS_SET_BROADCAST_CODE
    };
...
{
    struct ble_audio_event ev = {
        .type = BLE_AUDIO_EVENT_BASS_REMOVE_SOURCE
    };
    struct ble_svc_audio_bass_rcv_state_entry *rcv_state = NULL;
    int i;
...
ble_svc_audio_bass_ctrl_point_write_access(struct ble_gatt_access_ctxt *ctxt, uint16_t conn_handle)
{
    ...
    return handler->handler_cb(&ctxt->om->om_data[1], conn_handle);
}
...
ble_svc_audio_bass_rcv_state_read_access(struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    os_mbuf_append(ctxt->om, &state->source_addr.type, 1);
    os_mbuf_append(ctxt->om, &state->source_addr.val, 6);
    os_mbuf_append(ctxt->om, &state->source_adv_sid, 1);
    buf = os_mbuf_extend(ctxt->om, 3);
    if (buf == NULL) {
        return BLE_ATT_ERR_INSUFFICIENT_RES;
    }

    put_le24(buf, state->broadcast_id);
    os_mbuf_append(ctxt->om, &state->pa_sync_state, 1);
    os_mbuf_append(ctxt->om, &state->big_encryption, 1);

    if (state->big_encryption == BLE_SVC_AUDIO_BASS_BIG_ENC_BAD_CODE) {
        os_mbuf_append(ctxt->om, &state->bad_code, BLE_AUDIO_BROADCAST_CODE_SIZE);
    }

    os_mbuf_append(ctxt->om, &state->num_subgroups, 1);

    for (i = 0; i < state->num_subgroups; i++) {
        buf = os_mbuf_extend(ctxt->om, 4);
        if (buf == NULL) {
            return BLE_ATT_ERR_INSUFFICIENT_RES;
        }

        put_le32(buf, state->subgroups[i].bis_sync_state);
        os_mbuf_append(ctxt->om, &state->subgroups[i].metadata_length, 1);
        os_mbuf_append(ctxt->om, state->subgroups[i].metadata,
...
int
ble_svc_audio_bass_receive_state_add(const struct ble_svc_audio_bass_receiver_state *state,
                                     uint8_t *source_id)
{
    ...
    rcv_state->source_id = ble_svc_audio_bass_get_new_source_id();
    memcpy(&rcv_state->state, state, sizeof(struct ble_svc_audio_bass_receiver_state));
    ...
    for (i = 0; i < rcv_state->state.num_subgroups; i++) {
        ...
        rcv_state->state.subgroups[i].metadata_length =
            min(state->subgroups[i].metadata_length,
                ble_audio_svc_bass_metadata_pool.mp_block_size);
        memcpy(rcv_state->state.subgroups[i].metadata, state->subgroups[i].metadata,
               rcv_state->state.subgroups[i].metadata_length);
    }
...
int
ble_svc_audio_bass_update_sync(const struct ble_svc_audio_bass_sync_params *params,
                               uint8_t source_id)
{
    struct ble_svc_audio_bass_rcv_state_entry *rcv_state = NULL;
    int rc;
...
int
ble_svc_audio_bass_update_enc(const struct ble_svc_audio_bass_enc_params *params,
                              uint8_t source_id)
{
    struct ble_svc_audio_bass_rcv_state_entry *rcv_state = NULL;
    int rc;
    ...
    if (params->bad_code) {
        memcpy(rcv_state->state.bad_code, params->bad_code, BLE_AUDIO_BROADCAST_CODE_SIZE);
    }
...
int
ble_svc_audio_bass_update_bis_sync(const struct ble_svc_audio_bass_bis_sync_params *params,
                                   uint8_t source_id)
{
    struct ble_svc_audio_bass_rcv_state_entry *rcv_state = NULL;
    int rc;
    ...
    rcv_state->state.subgroups[params->subgroup_idx].bis_sync_state = params->bis_sync_state;
...
ble_svc_audio_bass_init(void)
{
    ...
    ble_svc_audio_bass_chrs[0].uuid =
    ...
    for (i = 1; i <= MYNEWT_VAL(BLE_SVC_AUDIO_BASS_RECEIVE_STATE_MAX); i++) {
        ble_svc_audio_bass_chrs[i].uuid =
        ...
        ble_svc_audio_bass_chrs[i].flags = BLE_GATT_CHR_F_READ_ENC | BLE_GATT_CHR_F_NOTIFY;
    }
