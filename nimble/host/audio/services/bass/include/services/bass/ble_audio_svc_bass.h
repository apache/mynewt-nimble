...
#define BLE_SVC_AUDIO_BASS_SUB_NUM_MAX                                   0x1F
...
struct ble_svc_audio_bass_sync_params {
    /** PA Sync state */
    enum ble_svc_audio_bass_pa_sync_state pa_sync_state;
};

struct ble_svc_audio_bass_enc_params {
...
struct ble_svc_audio_bass_bis_sync_params {
    /** Subgroup index */
    uint8_t subgroup_idx;
...
struct ble_svc_audio_bass_operation {
    ...
    union {
        ...
        struct {
            ...
            /**
             * Pointer to Source ID identifying Receiver State that will
             * be filled with accepted value from Add Source operation.
             * This shall be filled by accept function. If left as
             * BLE_SVC_AUDIO_BASS_RECEIVE_STATE_SRC_ID_NONE, BASS server will
             * find new value and assign it to new Reveive State. If this
             * is filled with other value, Accept
             */
            uint8_t *source_id;
        } add_source;
 ...
 *
 * Set Accept Function callback that will be used to accept or reject operations queried on
 * BASS Control Point characteristic. If no function is registered, operations are
 * accepted by default. Only one Accept Function can be registered at once.
 *
 * @param[in] fn                        ble_svc_audio_bass_accept_fn to be registered.
 * @param[in] arg                       Optional ble_svc_audio_bass_accept_fn argument.
 *
 * @return                              0 on success;
 ...
 *
 * @param[in] params                     Parameters of new Broadcast Receive State.
 * @param[out] source_id                Source ID assigned by BASS to new Broadcast Receive State
 *
 * @return                              0 on success;
 *                                      A non-zero value on failure.
 */
 ...
 *
 * @param[in] source_id                 Source ID of Broadcast Receive State to be removed
 *
 * @return                              0 on success;
 *                                      A non-zero value on failure.
 */
...
/**
 * @brief Update Broadcast Receive State PA Sync State.
 *
 * Set Broadcast Receive State PA Sync State to new value.
 *
 * @param[in] params                   Parameter structure with new PA Sync State.
 * @param[in] source_id                Source ID of Broadcast Receive State
 *
 * @return                              0 on success;
 *                                      A non-zero value on failure.
...
/**
 * @brief Update Broadcast Receive State BIG Encryption.
 *
 * Set Broadcast Receive State BIG Encryption to new value.
 *
 * @param[in] params                   Parameter structure with new BIG Encryption.
 * @param[in] source_id                Source ID of Broadcast Receive State
 *
 * @return                              0 on success;
 *                                      A non-zero value on failure.
...
/**
 * @brief Update Broadcast Receive State BIS Sync State.
 *
 * Set Broadcast Receive State BIS Sync State to new value.
 *
 * @param[in] params                   Parameter structure with new BIS Sync State.
 * @param[in] source_id                Source ID of Broadcast Receive State
 *
 * @return                              0 on success;
 *                                      A non-zero value on failure.
 ...
 *
 * @param[in] source_id                Source ID of Broadcast Receive State
 * @param[out] state                   Pointer to Broadcast Receive State characteristic value
 *
 * @return                              0 on success;
 *                                      A non-zero value on failure.
 */
