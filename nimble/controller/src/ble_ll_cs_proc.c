...
#define BLE_LL_CS_TONE_QUALITY_HIGH    (0)
#define BLE_LL_CS_TONE_QUALITY_MEDIUM  (1)
#define BLE_LL_CS_TONE_QUALITY_LOW     (2)
#define BLE_LL_CS_TONE_QUALITY_UNKNOWN (3)
...
#define BLE_LL_CS_RTT_SEQUENCE_MAX (16)
#define BLE_LL_CS_RTT_STEPS_PER_SUBEVENT_MAX (10)
#define POOL_RTT_COUNT (BLE_LL_CS_RTT_STEPS_PER_SUBEVENT_MAX * 2)
#define POOL_RTT_SIZE (BLE_LL_CS_RTT_SEQUENCE_MAX)

static os_membuf_t pool_rtt_buf[OS_MEMPOOL_SIZE(POOL_RTT_COUNT, POOL_RTT_SIZE)];
static struct os_mempool pool_rtt;

/* A pattern containing the transmissions of the pending subevent */
...
static void ble_ll_cs_proc_sync_lost(struct ble_ll_cs_sm *cssm);
...
ble_ll_cs_tone_quality_indicator(int16_t I12, int16_t Q12)
{
    ...
    if (amplitude < 10) return BLE_LL_CS_TONE_QUALITY_UNKNOWN;
    if (amplitude > 1500) return BLE_LL_CS_TONE_QUALITY_HIGH;
    if (amplitude > 800) return BLE_LL_CS_TONE_QUALITY_MEDIUM;
    return BLE_LL_CS_TONE_QUALITY_LOW;
}

static void
ble_ll_cs_tone_results_set(struct ble_ll_cs_step *step,
                           struct ble_phy_cs_tone_results *results)
{
    ...
    if (step->mode == BLE_LL_CS_MODE0) {
        ...
        for (i = cssm->step_result.tone_count; i < ARRAY_SIZE(cssm->step_result.tone_pct); ++i) {
            ...
            cssm->step_result.tone_quality_ind[i] = BLE_LL_CS_TONE_QUALITY_UNKNOWN;
        }
    }
}
...
int
ble_ll_cs_csa3b(struct ble_ll_cs_drbg_ctx *drbg_ctx, uint16_t step_count, uint8_t transaction_id,
                uint8_t *channel_array, uint8_t *shuffled_array, uint8_t len)
{
    return ble_ll_cs_drbg_shuffle_cr1(drbg_ctx, step_count, transaction_id, channel_array,
                                      shuffled_array, len);
}
                           ...
                           uint16_t steps_in_procedure_count)
{
    ...
    if (conf->filtered_channels_count <= *next_channel_id) {
        if (conf->chan_sel == BLE_LL_CS_CSA_TYPE_3B) {
            rc = ble_ll_cs_csa3b(&cssm->drbg_ctx, steps_in_procedure_count,
                                 transaction_id, conf->filtered_channels,
                                 channel_array, conf->filtered_channels_count);
        } else {
#if MYNEWT_VAL(BLE_LL_CHANNEL_SOUNDING_CSA3C)
            rc = ble_ll_cs_csa3c(&cssm->drbg_ctx, channel_array, conf->filtered_channels,
                                 &cssm->csa3c_start_jitter, &cssm->ch3c_shape_iter,
                                 steps_in_procedure_count, conf->ch3cshape, conf->ch3cjump,
                                 conf->chan_map_repetition);
#else
            BLE_LL_ASSERT(0);
#endif
        }
...
ble_ll_cs_backtracking_resistance(struct ble_ll_cs_sm *cssm)
{
    ...
    return ble_ll_cs_drbg_f9(NULL, cssm->drbg_ctx.key, cssm->drbg_ctx.nonce_v);
}
                        ...
                        struct ble_ll_cs_sm *cssm)
{
    ...
    ev->procedure_counter = cssm->pending_procedure_id;
    ev->frequency_compensation = 0xC000;
    ev->reference_power_level = g_ble_ll_tx_power;
...
int
ble_ll_cs_rtt_generate(struct ble_ll_cs_drbg_ctx *drbg_ctx, struct ble_phy_cs_transmission *transm,
                       uint16_t steps_in_procedure_count, uint8_t rtt_type)
{
    int rc;

    transm->rtt_sequence = os_memblock_get(&pool_rtt);

    BLE_LL_ASSERT(transm->rtt_sequence != NULL);

    memset(transm->rtt_sequence, 0, POOL_RTT_SIZE);

    rc = ble_ll_cs_drbg_generate_sync_sequence(drbg_ctx, steps_in_procedure_count, rtt_type,
                                               transm->rtt_sequence, &transm->rtt_sequence_len);

    return rc;
}

static int
ble_ll_cs_proc_step_transmission_generate(struct ble_ll_cs_drbg_ctx *drbg_ctx,
                                          struct ble_ll_cs_step *prev_step,
                                          struct ble_ll_cs_step *step,
                                          struct ble_ll_cs_step_transmission *last_transm_slot,
                                          uint16_t steps_in_procedure_count,
                                          uint8_t role, uint8_t n_ap, uint8_t t_sy,
                                          uint8_t t_pm, uint8_t t_sy_seq, uint8_t rtt_type)
{
    ...
    while (next_state != STEP_STATE_COMPLETE) {
        ...
        if (next_state == STEP_STATE_CS_TONE_I || next_state == STEP_STATE_CS_TONE_R) {
            ...
            --slot_count;
        } else if (next_state == STEP_STATE_CS_SYNC_I || next_state == STEP_STATE_CS_SYNC_R) {
            if (rtt_type != BLE_LL_CS_RTT_AA_ONLY && step->mode != BLE_LL_CS_MODE0) {
                ble_ll_cs_rtt_generate(drbg_ctx, &transm->phy_transm, steps_in_procedure_count, rtt_type);
            }
        }
                        ...
                        uint8_t subevent_state, uint16_t steps_in_procedure_count)
{
    ...
    ble_ll_cs_proc_step_transmission_generate(&cssm->drbg_ctx, cssm->last_step, step, last_transm_slot,
                                              steps_in_procedure_count, conf->role, cssm->n_ap,
                                              cssm->t_sy, conf->t_pm, cssm->t_sy_seq, conf->rtt_type);
...
{
    uint32_t anchor_ticks = cssm->procedure_anchor_ticks;
    uint8_t anchor_rem_us = cssm->procedure_anchor_rem_us;

    cssm->steps_in_subevent_count = 0;

    ble_ll_tmr_add(&anchor_ticks, &anchor_rem_us,
                   cssm->events_in_procedure_count * cssm->event_interval_usecs +
                   cssm->subevents_in_event_count * cssm->subevent_interval_usecs);

    cssm->anchor_ticks = anchor_ticks;
    cssm->anchor_rem_us = anchor_rem_us;
...
ble_ll_cs_setup_next_procedure(struct ble_ll_cs_sm *cssm)
{
    ...
    if (conf->proc_params.max_procedure_count != BLE_LL_CS_PROCEDURE_COUNT_NO_LIMIT &&
        cssm->pending_procedure_id - cssm->first_procedure_id + 1 >= conf->proc_params.max_procedure_count) {
        /* All CS procedures in the repeat series have been completed */
        cssm->cs_schedule_status = CS_SCHEDULE_COMPLETED;
        return 1;
    }

    if (cssm->terminate_measurement && cssm->terminate_procedure_id <= cssm->pending_procedure_id) {
    ...
    anchor_ticks = cssm->procedure_anchor_ticks;
    anchor_rem_us = cssm->procedure_anchor_rem_us;

    ble_ll_tmr_add(&anchor_ticks, &anchor_rem_us,
                   (cssm->pending_procedure_id - cssm->first_procedure_id) *
                    cssm->procedure_interval_usecs);
...
ble_ll_cs_procedure_duration_get(struct ble_ll_cs_sm *cssm)
{
    uint32_t procedure_duration_us;
    uint32_t anchor_ticks;
    uint8_t anchor_rem_us;

    assert(cssm->anchor_ticks > cssm->procedure_anchor_ticks ||
           (cssm->anchor_ticks == cssm->procedure_anchor_ticks &&
            cssm->anchor_rem_us >= cssm->procedure_anchor_rem_us));

    anchor_ticks = cssm->anchor_ticks - cssm->procedure_anchor_ticks;
    anchor_rem_us = cssm->anchor_rem_us;
    ble_ll_tmr_sub(&anchor_ticks, &anchor_rem_us, cssm->procedure_anchor_rem_us);

    procedure_duration_us = ble_ll_tmr_t2u(anchor_ticks) + anchor_rem_us;

    return procedure_duration_us;
}
...
ble_ll_cs_proc_subevent_generate(struct ble_ll_cs_sm *cssm)
{
    ...
    procedure_duration = ble_ll_cs_procedure_duration_get(cssm);
    ...
    cssm->estimated_subevent_len = subevent_duration;
...
ble_ll_cs_proc_subevent_schedule(struct ble_ll_cs_sm *cssm)
{
    ...
    ble_ll_tmr_add(&cssm->anchor_ticks, &cssm->anchor_rem_us, conf->t_fcs);
...
ble_ll_cs_proc_schedule_next_subevent(struct ble_ll_cs_sm *cssm)
{
    ...
    if (cssm->anchor_ticks - g_ble_ll_sched_offset_ticks < ble_ll_tmr_get()) {
        return 1;
    }

    cssm->sched_cb = ble_ll_cs_proc_subevent_schedule;
    cssm->sch.start_time = cssm->anchor_ticks - g_ble_ll_sched_offset_ticks;
    cssm->sch.end_time = cssm->anchor_ticks + ble_ll_tmr_u2t_up(cssm->estimated_subevent_len);
    cssm->sch.remainder = cssm->anchor_rem_us;
...
ble_ll_cs_proc_schedule_first_subevent(struct ble_ll_cs_sm *cssm)
{
    ...
    cssm->anchor_ticks = anchor_ticks;
    cssm->anchor_rem_us = anchor_rem_usecs;
    cssm->n_ap = aci_table[params->aci].n_ap;
    cssm->procedure_anchor_ticks = cssm->anchor_ticks;
    cssm->procedure_anchor_rem_us = cssm->anchor_rem_us;

    cssm->current_step = NULL;
    cssm->last_step = NULL;

    if (cssm->next_procedure_id > 0) {
        ble_ll_cs_backtracking_resistance(cssm);
    }

    cssm->pending_procedure_id = cssm->next_procedure_id;
    cssm->first_procedure_id = cssm->pending_procedure_id;
    cssm->next_procedure_id++;
...
ble_ll_cs_subevent_end(struct ble_phy_cs_subevent_results *results)
{
    ...
    for (i = 0; i < ARRAY_SIZE(subevent_steps); ++i) {
        step = &subevent_steps[i];
        transm = step->next_transm;
        if (transm == NULL) {
            continue;
        }

        phy_transm = &transm->phy_transm;
        while (phy_transm != NULL) {
            if (phy_transm->rtt_sequence) {
                os_memblock_put(&pool_rtt, phy_transm->rtt_sequence);
            }
            phy_transm = phy_transm->next;
        }
    }
...
void
ble_ll_cs_proc_init(void)
{
    int rc;

    rc = os_mempool_init(&pool_rtt, POOL_RTT_COUNT, POOL_RTT_SIZE,
                         pool_rtt_buf, "cs_rtt_pool_cmd");
    BLE_LL_ASSERT(rc == 0);
}
