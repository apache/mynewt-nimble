...
ble_ll_cs_drbg_e(const uint8_t *key, const uint8_t *data, uint8_t *out_data)
{
    ...
    rc = mbedtls_aes_crypt_ecb(&aes_ctx, MBEDTLS_AES_ENCRYPT, ecb.plain_text, ecb.cipher_text);
 ...
 * temporal key K and a 128-bit nonce vector V using a 256-bit seed material (SM)
 * that may carry fresh entropy. The SM value may also be 0 if f9 is called for
 ...
 */
int
ble_ll_cs_drbg_rand(struct ble_ll_cs_drbg_ctx *drbg_ctx,
                    uint16_t step_count, uint8_t transaction_id,
                    uint8_t *output, uint8_t len)
{
    int rc;
    uint8_t rand_len = 0;
    uint8_t nonce[16];
                                     ...
                                     uint8_t step_count, uint8_t *marker_selection)
{
    ...
    if (drbg_ctx->marker_selection_free_bits == 0) {
        rc = ble_ll_cs_drbg_rand(drbg_ctx, step_count,
                                 BLE_LL_CS_DRBG_SEQ_MARKER_SIGNAL,
...
static int
ble_ll_cs_drbg_apply_marker_signal(struct ble_ll_cs_drbg_ctx *drbg_ctx, uint8_t step_count,
                                      ...
                                      uint8_t *sequence_len)
{
    ...
    *sequence_len = rtt_seq_len[rtt_type];
    ...
    case BLE_LL_CS_RTT_32_BIT_SOUNDING_SEQUENCE:
        rc = ble_ll_cs_generate_sounding_sequence(drbg_ctx, step_count, rtt_type,
                                                  buf, *sequence_len);
        break;
    ...
    case BLE_LL_CS_RTT_128_BIT_RANDOM_SEQUENCE:
        rc = ble_ll_cs_drbg_rand(drbg_ctx, step_count,
                                 BLE_LL_CS_DRBG_RAND_SEQ_GENERATION,
                                 buf, *sequence_len);
        break;
    default:
        break;
