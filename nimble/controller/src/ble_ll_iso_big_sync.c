...
#define BIS_POOL_SIZE           (MYNEWT_VAL(BLE_LL_ISO_BROADCAST_SYNC_MAX_BIG) * \
...
big_sync_sched_set(struct ble_ll_iso_big *big, uint32_t offset_us)
{
    ...
    ww_us = ble_ll_utils_calc_window_widening(big->sch.start_time, big->anchor_base_ticks, big->sca);
    if (ww_us >= max_ww_us) {
        return -1;
    }
...
ble_ll_iso_big_sync_event_sched_cb(struct ble_ll_sched_item *sch)
{
    ...
    STAILQ_FOREACH(bis, &big->bis_q, bis_q_next) {
        ble_ll_iso_conn_event_start(&bis->conn, timestamp);
...
ble_ll_iso_big_sync_rx_isr_start(uint8_t pdu_type, struct ble_mbuf_hdr *rxhdr)
{
    ...
    if (big->subevents_rem == big->num_bis * big->nse) {
        big->anchor_offset = 0;
...
ble_ll_iso_big_sync_rx_restart(struct ble_ll_iso_big *big)
{
    ...
    return ble_ll_iso_big_sync_rx_start(big, ticks + g_ble_ll_sched_offset_ticks, rem_us);
}
