             ...
             uint8_t sync_rem_us, const uint8_t *data, uint8_t len, void *arg)
{
    ...
    ble_ll_tmr_add(&big->anchor_base_ticks, &big->anchor_base_rem_us,
                   big_offset_us + offset_us);
...
ble_ll_iso_big_sync_create(uint8_t big_handle, struct ble_ll_iso_big **out)
{
    ...
    big = big_ll_iso_big_sync_find(big_handle);
    if (big) {
        return -EALREADY;
    }
...
ble_ll_iso_big_sync_rx_pdu_in(struct os_mbuf **rxpdu, struct ble_mbuf_hdr *rxhdr)
{
    ...
    big = big_ll_iso_big_sync_find(pdu_user_data.big_handle);
    if (big == NULL) {
...
big_ll_iso_big_sync_find(uint8_t big_handle)
{
    struct ble_ll_iso_big *big = NULL;
