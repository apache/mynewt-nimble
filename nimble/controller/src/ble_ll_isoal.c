...
static uint8_t
ble_ll_isoal_num_pdu_get(struct ble_ll_isoal_mux *mux)
{
    if (mux->framed) {
        return ble_ll_isoal_framed_pdu_num(mux);
    } else {
        return mux->sdu_q_len * mux->pdu_per_sdu;
    }
}
...
ble_ll_isoal_mux_sdu_enqueue(struct ble_ll_isoal_mux *mux, struct os_mbuf *om)
{
    ...
    if (ble_ll_isoal_num_pdu_get(mux) >= mux->pdu_per_event) {
        mux->active = 1;
    }
#endif
    OS_EXIT_CRITICAL(sr);
...
{
    uint8_t num_pdu = ble_ll_isoal_num_pdu_get(mux);
