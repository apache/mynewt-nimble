...
void
ble_ll_cs_req_make(struct ble_ll_conn_sm *connsm, uint8_t *dptr)
{
    uint8_t config_id = connsm->cssm->config_req_id;
    uint8_t subevents_per_event;
    uint16_t anchor_conn_event = connsm->event_cntr + MIN_CONN_EVENT_COUNT_BEFORE_START;
    struct ble_ll_cs_pref_proc_params *params;
    ...
    *dptr = config_id;
    put_le16(dptr + 1, anchor_conn_event);
...
static int
ble_ll_cs_ind_make(struct ble_ll_conn_sm *connsm, uint8_t *rspbuf)
{
    uint8_t config_id = connsm->cssm->config_req_id;
    struct ble_ll_cs_proc_params *ps;
    ...
    *rspbuf = config_id;
    put_le16(rspbuf + 1, ps->anchor_conn_event);
...
static int
ble_ll_cs_rsp_make(struct ble_ll_conn_sm *connsm, uint8_t *rspbuf)
{
    uint8_t config_id = connsm->cssm->config_req_id;
    struct ble_ll_cs_proc_params *ps;
                             ...
                             struct ble_ll_cs_proc_params *ps)
{
    ...
    if (ps->anchor_conn_event < connsm->event_cntr + 3) {
         ...
         */
        ps->anchor_conn_event = connsm->event_cntr + 3;
        err = 1;
    }
...
int
ble_ll_cs_rx_cs_req(struct ble_ll_conn_sm *connsm, uint8_t *dptr, uint8_t *rspbuf)
{
    uint8_t config_id = *dptr & 0b00111111;
    struct ble_ll_cs_config *conf;
    struct ble_ll_cs_proc_params *ps;
    ...
    if (config_id >= ARRAY_SIZE(connsm->cssm->config)) {
        return rej_ext_ind_make(BLE_LL_CTRL_CS_REQ,
                                BLE_ERR_INV_LMP_LL_PARM, rspbuf);
    }
    ...
    if (CONN_IS_CENTRAL(connsm) &&
        conf->role != BLE_LL_CS_ROLE_REFLECTOR) {
        return rej_ext_ind_make(BLE_LL_CTRL_CS_REQ,
    ...
    ps->anchor_conn_event = get_le16(dptr + 1);
    ...
    if (CONN_IS_CENTRAL(connsm)) {
        rc = ble_ll_cs_proc_scheduling_start(connsm, conf);
        if (rc) {
            memset(ps, 0, sizeof(*ps));
            return rej_ext_ind_make(BLE_LL_CTRL_CS_REQ, rc, rspbuf);
        }

        ble_ll_cs_ev_cs_proc_enable_complete(connsm, config_id, BLE_ERR_SUCCESS);

        return ble_ll_cs_ind_make(connsm, rspbuf);
    }

    return ble_ll_cs_rsp_make(connsm, rspbuf);
}

int
ble_ll_cs_rx_cs_rsp(struct ble_ll_conn_sm *connsm, uint8_t *dptr, uint8_t *rspbuf)
{
    int rc;
    struct ble_ll_cs_config *conf;
    struct ble_ll_cs_proc_params *ps;
    ...
    if (config_id != connsm->cssm->config_req_id ||
        !IS_PENDING_CTRL_PROC(connsm, BLE_LL_CTRL_PROC_CS_START)) {
        return rej_ext_ind_make(BLE_LL_CTRL_CS_RSP,
                                BLE_ERR_INV_LMP_LL_PARM, rspbuf);
    }
    ...
    if (!CONN_IS_CENTRAL(connsm) ||
        conf->role != BLE_LL_CS_ROLE_INITIATOR) {
    ...
    if (rc) {
        memset(ps, 0, sizeof(*ps));
        return rej_ext_ind_make(BLE_LL_CTRL_CS_RSP, rc, rspbuf);
    }
...
int
ble_ll_cs_rx_cs_ind(struct ble_ll_conn_sm *connsm, uint8_t *dptr,
                    uint8_t *rspbuf)
{
    int rc;
    uint8_t config_id = *dptr & 0b00111111;
    ...
    if (config_id != connsm->cssm->config_req_id) {
        return rej_ext_ind_make(BLE_LL_CTRL_CS_IND,
                                BLE_ERR_INV_LMP_LL_PARM, rspbuf);
    }
    ...
    if (rc) {
        memset(ps, 0, sizeof(*ps));
        return rej_ext_ind_make(BLE_LL_CTRL_CS_IND, rc, rspbuf);
    }
