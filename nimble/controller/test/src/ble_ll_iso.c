...
static void
test_ll_setup(struct test_ll_fixture *fixture, const struct test_ll_common_params *params)
{
    ...
    mux_init_param.iso_interval_us = params->ISO_Interval * 1000;
    mux_init_param.sdu_interval_us = params->SDU_Interval * 1000;
    mux_init_param.max_sdu = TSPX_max_tx_payload;
    mux_init_param.max_pdu = TSPX_max_tx_payload;
    mux_init_param.bn = params->BN;
    mux_init_param.pte = 0;
    mux_init_param.framed = params->Framing != 0x00;
    mux_init_param.framing_mode = params->Framing == 0x02;
    ble_ll_isoal_mux_init(&conn->mux, &mux_init_param);

    ble_ll_iso_conn_add(conn);
}

static void
test_ll_teardown(struct test_ll_fixture *fixture)
{
    struct ble_ll_iso_conn *conn;
...
TEST_CASE_SELF(test_ll_ist_brd_bv_01_c) {
    ...
    struct test_ll_fixture fixture;
    struct ble_ll_iso_conn *conn;
...
TEST_CASE_SELF(test_ll_ist_snc_bv_01_c) {
    ...
    for (uint8_t i = 0; i < ARRAY_SIZE(rounds); i++) {
        ...
        /* 3. The Upper Tester sends the HCI_LE_ISO_Receive_Test command to the IUT with
         *    Payload_Type as specified in Table 4.12-4 and receives a successful HCI_Command_Complete
         *    event from the IUT in response.
         */
        rsplen = 0xFF;
        iso_receive_test_cp.conn_handle = htole16(conn->handle);
        iso_receive_test_cp.payload_type = payload_type;
        rc = ble_ll_iso_receive_test((uint8_t *)&iso_receive_test_cp, sizeof(iso_receive_test_cp),
        ...
        for (uint8_t j = 0; j < 5; j++) {
            ...
            /* 5. TODO The IUT receives the expected test Payload PDU as specified in Table 4.12-4. */
        }
        ...
        rc = ble_ll_iso_read_counters_test((uint8_t *)&iso_read_test_counters_cp,
                                           ...
                                           (uint8_t *)&iso_read_test_counters_rp, &rsplen);
        TEST_ASSERT(rc == 0);
        TEST_ASSERT(rsplen == sizeof(iso_read_test_counters_rp));
        TEST_ASSERT(iso_read_test_counters_rp.conn_handle == iso_read_test_counters_cp.conn_handle);

        /* 8. TODO The Upper Tester receives a return parameter of Received_SDU_Count as specified
         *    in Table 4.12-4.
         */
