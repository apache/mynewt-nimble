                  ...
                  const struct test_ll_common_params *params)
{
    ...
    iso_params->max_sdu = params->Max_SDU;
...
TEST_CASE_SELF(test_ll_ist_snc_bv_01_c)
{
    const struct test_ll_iso_receive_test_mode_round rounds[] = {
        { 0, BLE_HCI_PAYLOAD_TYPE_ZERO_LENGTH },
        { 0, BLE_HCI_PAYLOAD_TYPE_VARIABLE_LENGTH },
        { 0, BLE_HCI_PAYLOAD_TYPE_MAXIMUM_LENGTH },
        { 1, BLE_HCI_PAYLOAD_TYPE_MAXIMUM_LENGTH },
        { 1, BLE_HCI_PAYLOAD_TYPE_VARIABLE_LENGTH }
    };
    struct test_ll_common_params params = test_ll_common_params_bn_1;
    ...
    for (uint8_t i = 0; i < ARRAY_SIZE(rounds); i++) {
        framed = rounds[i].framing;
        if (framed) {
            params.Framing = 0x01;
            params.Max_SDU = TSPX_max_tx_payload - 5;
        }
        ...
        conn = &fixture.conn;
        demux = &conn->rx->demux;
        max_sdu = fixture.params.max_sdu;
        ...
        for (uint8_t j = 0; j < 5; j++) {
            /* The first 4 octets of each SDU contain an SDU Count that
             * increments with each received SDU, in LSB order.
             */
            put_le16(sdu, sdu_count++);

            if (payload_type == BLE_HCI_PAYLOAD_TYPE_ZERO_LENGTH) {
                dlen = 0;
            } else if (payload_type == BLE_HCI_PAYLOAD_TYPE_VARIABLE_LENGTH) {
                dlen = 4 + rand() % (max_sdu - 4);
            } else if (payload_type == BLE_HCI_PAYLOAD_TYPE_MAXIMUM_LENGTH) {
                dlen = max_sdu;
            }

            /* 4. The Lower Tester sends an SDU as specified in Table 4.12-4. */
            /* 5. The IUT receives the expected test Payload PDU as specified in Table. */
            ble_ll_isoal_demux_event_start(demux, (i + j) * params.ISO_Interval);

            if (framed) {
                (void)test_ial_lt_fra_pdu_send(demux, 0, sdu, dlen, params.ISO_Interval, true, true);
            } else {
                (void)test_ial_lt_unf_pdu_send(demux, 0, sdu, dlen, 0b00);
            }
        ...
        /* 8. The Upper Tester receives a return parameter of
         *    Received_SDU_Count as specified in Table 4.12-4.
         */
        TEST_ASSERT(iso_read_test_counters_rp.received_sdu_count == 5);
