...
static struct job_entry input_job_list[2];
static struct job_entry output_job_list[2];
...
ble_hw_encrypt_block(struct ble_encryption_block *ecb)
{
    ...
    input_job_list[0].ptr = ecb->plain_text;
    input_job_list[0].attr_and_length = (11 << 24) | (16 & 0x00ffffff);
    output_job_list[0].ptr = ecb->cipher_text;
    output_job_list[0].attr_and_length = (11 << 24) | (16 & 0x00ffffff);

    /* The end of a job list shall be 0 */
    input_job_list[1].ptr = 0;
    input_job_list[1].attr_and_length = 0;
    output_job_list[1].ptr = 0;
    output_job_list[1].attr_and_length = 0;

    NRF_ECB->EVENTS_END = 0;
    NRF_ECB->EVENTS_ERROR = 0;
    NRF_ECB->IN.PTR = (uint32_t)input_job_list;
    NRF_ECB->OUT.PTR = (uint32_t)output_job_list;
