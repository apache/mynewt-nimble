...
#if MYNEWT_VAL(BLE_PHY_CHANNEL_SOUNDING)
                         ...
                         uint8_t role)
{
    ...
    nrf_timer_cc_set(NRF_TIMER10, 0, 0xffffffff);
    nrf_timer_cc_set(NRF_TIMER10, 1, 0xffffffff);
    ...
    nrf_timer_cc_set(NRF_TIMER10, 6, 0xffffffff);
...
ble_phy_cs_subevent_start(struct ble_phy_cs_transmission *transm,
                          uint32_t cputime, uint8_t rem_usecs, uint8_t phy_mode)
{
    int rc;
    ...
    g_ble_phy_cs.phy_mode = phy_mode;
     ...
     * Set the preambule length to 8 bits for LE 1M and 16 bits for LE 2M.
    ...
    if (phy_mode == BLE_PHY_MODE_1M) {
        ...
        NRF_RADIO->PCNF0 = (RADIO_PCNF0_PLEN_16bit << RADIO_PCNF0_PLEN_Pos)
                           | (4 << RADIO_PCNF0_S1LEN_Pos);
    } else {
        assert(0);
    }
