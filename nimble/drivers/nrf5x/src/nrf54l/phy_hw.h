...
/* AAR_ATTR_OUTPUT uses attr ID 11 on OUT.PTR (same numeric value as
 * AAR_ATTR_HASH on IN.PTR; in/out lists are independent channels per
 * datasheet §8.3 Fig. 41-42). The resolved IRK index is written as a
 * 4-byte value matching Zephyr's reference; see nrf54l/phy.c. */
...
int phy_hw_ccm_tx_wait_complete(void); /* returns 0 on success, -1 on timeout */
int phy_hw_ccm_finish_rx_decrypt(uint8_t *enc_buf, uint8_t *out_buf, uint8_t enc_len);
