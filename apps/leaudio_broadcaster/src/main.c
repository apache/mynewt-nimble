...
static struct ble_audio_base broadcaster_base;
...
static os_stack_t broadcaster_interrupt_task_stack[BROADCASTER_INTERRUPT_TASK_STACK_SZ];
...
{
    ble_audio_broadcast_stop(BROADCAST_ADV_INSTANCE);
    ble_audio_broadcast_destroy(BROADCAST_ADV_INSTANCE);
}
...
static void
broadcaster_gpio_irq(void *arg)
{
    os_eventq_put(&broadcaster_interrupt_eventq, &broadcast_stop_ev);
}
...
base_create()
{
    ...
    STAILQ_INSERT_HEAD(&broadcaster_base.subs, &big_subgroup, next);
...
mynewt_main(int argc, char **argv)
{
    ...
    os_eventq_init(&broadcaster_interrupt_eventq);
    os_task_init(&broadcaster_interrupt_task_str, "broadcaster_interrupt_task",
                 broadcaster_interrupt_task, NULL,
                 BROADCASTER_INTERRUPT_TASK_PRIO, OS_WAIT_FOREVER,
                 broadcaster_interrupt_task_stack,
                 BROADCASTER_INTERRUPT_TASK_STACK_SZ);

    hal_gpio_irq_init(BUTTON_3, broadcaster_gpio_irq, NULL,
