...
bletest_send_ltk_req_neg_reply(uint16_t handle)
{
    ...
    rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_LE,
                           ...
                           buf, sizeof(uint16_t), &ack_conn_handle, 2,
                           &rsplen);
    if (rc == 0) {
        if (rsplen != 2) {
            rc = -1;
...
bletest_hci_le_rd_sugg_datalen(void)
{
    ...
    rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_LE,
                           ...
                           NULL, 0, rspbuf, BLE_HCI_RD_SUGG_DATALEN_RSPLEN,
                           &rsplen);
    if (rc != 0) {
        return rc;
    }
...
bletest_hci_rd_local_version(void)
{
    ...
    rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_INFO_PARAMS,
                           ...
                           NULL, 0, rspbuf, BLE_HCI_RD_LOC_VER_INFO_RSPLEN,
                           &rsplen);
    if (rc != 0) {
        return rc;
    }
...
bletest_hci_rd_local_feat(void)
{
    ...
    rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_INFO_PARAMS,
                           ...
                           NULL, 0, rspbuf, BLE_HCI_RD_LOC_SUPP_FEAT_RSPLEN,
                           &rsplen);
    if (rc != 0) {
        return rc;
    }
...
bletest_hci_rd_local_supp_cmd(void)
{
    ...
    rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_INFO_PARAMS,
                           ...
                           NULL, 0, rspbuf, BLE_HCI_RD_LOC_SUPP_CMD_RSPLEN,
                           &rsplen);
    if (rc != 0) {
        return rc;
    }
...
bletest_hci_le_read_supp_states(void)
{
    ...
    rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_LE,
                           ...
                           NULL, 0, rspbuf, BLE_HCI_RD_SUPP_STATES_RSPLEN,
                           &rsplen);
    if (rc != 0) {
        return rc;
    }
...
bletest_hci_le_rd_max_datalen(void)
{
    ...
    rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_LE,
                           ...
                           NULL, 0, rspbuf, BLE_HCI_RD_MAX_DATALEN_RSPLEN,
                           &rsplen);
    if (rc != 0) {
        return rc;
    }
...
bletest_hci_le_rd_chanmap(uint16_t handle)
{
    ...
    rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_LE,
                        ...
                        buf, sizeof(buf), rspbuf, BLE_HCI_RD_CHANMAP_RSP_LEN,
                        &rsplen);
    if (rc != 0) {
        return rc;
    }
...
bletest_hci_le_add_to_whitelist(uint8_t *addr, uint8_t addr_type)
{
    ...
    rc = ble_hs_hci_cmd_build_le_add_to_whitelist(addr, addr_type, buf,
                                                sizeof buf);
    opcode = BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_ADD_WHITE_LIST);
    if (!rc) {
        rc = ble_hs_hci_cmd_tx_empty_ack(opcode, buf, sizeof(buf));
    }
    return rc;
