...
cs_start_timer_reset(void)
{
    ...
    rc = os_callout_reset(&cs_start_timer, os_time_ms_to_ticks32(1000));
    assert(rc == 0);
}
...
cs_initiator_should_connect(const ble_addr_t *addr, const struct ble_hs_adv_fields *fields)
{
    ...
    if (fields->name != NULL && !memcmp((const char *)fields->name, "CS Reflector", 12)) {
        return 1;
    }
...
static void
cs_disc_all(uint16_t conn_handle)
{
    int rc;
...
cs_initiator_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_ENC_CHANGE:
        ...
        if (rc == BLE_HS_EALREADY) {
            cs_disc_all(event->enc_change.conn_handle);
        }
