...
tester_l2cap_event(struct ble_l2cap_event *event, void *arg)
{
	...
	switch (event->type) {
		...
		console_printf("LE COC connected, conn: %d, chan: %p, "
			       "psm: 0x%02x, scid: 0x%04x, dcid: 0x%04x, "
			       "our_mps: %d, our_mtu: %d, peer_mps: %d, "
			       "peer_mtu: %d\n", event->connect.conn_handle,
			       event->connect.chan, chan_info.psm,
	...
	case BLE_L2CAP_EVENT_COC_ACCEPT:
		...
		console_printf("LE CoC accept, chan: %p, handle: %u, sdu_size: %u\n",
			       event->accept.chan,
	...
	case BLE_L2CAP_EVENT_COC_DATA_RECEIVED:
		console_printf("LE CoC data received, chan: %p, handle: %u, sdu_len: %u\n",
			       event->receive.chan,
	...
	case BLE_L2CAP_EVENT_COC_TX_UNSTALLED:
		console_printf("LE CoC tx unstalled, chan: %p, handle: %u, status: %d\n",
